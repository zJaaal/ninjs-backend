 ...
 */
const count = async () => {
	const totalGenin = await Question.find({ difficult: 'Genin' }).count().exec();
	const totalChunin = await Question.find({ difficult: 'Chunin' })
		.count()
		.exec();
	const totalJonin = await Question.find({ difficult: 'Jonin' }).count().exec();
